...
const MAIN_MOUNTAIN =
  'M -10,560 L -10,534 ' +
  'C 24,518 60,500 98,480 ' +
  'C 136,460 174,440 212,418 ' +
  'C 244,400 262,384 280,370 ' +
  'C 296,354 314,360 332,370 ' +
  'C 352,380 374,360 408,328 ' +
  'C 430,308 446,284 460,265 ' +
  'C 474,248 492,256 512,266 ' +
  'C 534,278 562,256 596,226 ' +
  'C 622,204 642,176 660,150 ' +
  'C 676,130 698,138 718,150 ' +
  'C 740,162 778,112 818,74 ' +
  'C 840,58 854,48 860,48 ' +
  'C 874,48 898,40 920,52 ' +
  'C 942,64 964,82 988,108 ' +
  'L 1010,132 L 1010,560 Z';

// Rock texture strokes overlaid on the main mountain
const ROCK_STROKES = [
...
// Snow cap blobs (organic bezier shapes at each tier peak)
function SnowCap({ x, y, size }) {
  const s = size;
  return (
    <g>
      <path
        d={`M ${x-s*1.1},${y+s*1.6} C ${x-s*1.05},${y+s*0.8} ${x-s*0.6},${y-s*0.2} ${x},${y}
            C ${x+s*0.6},${y-s*0.2} ${x+s*1.05},${y+s*0.8} ${x+s*1.1},${y+s*1.6}
            C ${x+s*0.6},${y+s*1.1} ${x},${y+s*1.3} ${x-s*0.6},${y+s*1.1} Z`}
        fill="white" opacity="0.92"
      />
      <path
        d={`M ${x-s*0.7},${y+s*1.7} C ${x-s*0.6},${y+s*1.0} ${x-s*0.2},${y+s*0.3} ${x+s*0.2},${y+s*0.2}
            C ${x+s*0.6},${y+s*0.1} ${x+s*0.8},${y+s*0.9} ${x+s*0.7},${y+s*1.7} Z`}
...
// Drifting cloud shape
function Cloud({ x, y, scale = 1, cls = '' }) {
  return (
    <g transform={`translate(${x},${y}) scale(${scale})`} opacity="0.18" className={cls}>
...
// ── Sky gradient colors (shift with quarter progress) ────────────────────────
function lerp(a, b, t) { return a + (b - a) * t; }
function lerpColor(c1, c2, t) {
  return `rgb(${Math.round(lerp(c1[0],c2[0],t))},${Math.round(lerp(c1[1],c2[1],t))},${Math.round(lerp(c1[2],c2[2],t))})`;
}
function skyColors(frac) {
  const topA = [15,11,46], topB = [24,8,42];
  const botA = [26,18,66], botB = [72,24,40];
  return { top: lerpColor(topA, topB, frac), bottom: lerpColor(botA, botB, frac) };
}

// ── Main component ────────────────────────────────────────────────────────────
export default function Mountain({ board, streaks, badges }) {
  const frac = useMemo(() => quarterProgress(), []);
  const sky = useMemo(() => skyColors(frac), [frac]);
  ...
  return (
    <div className="mountain-wrap">
      ...
      >
        <defs>
          ...
          <radialGradient id="aurora" cx="75%" cy="-10%" r="55%">
            <stop offset="0%" stopColor="#1ee3cf" stopOpacity="0.13" />
            <stop offset="100%" stopColor="#1ee3cf" stopOpacity="0" />
          </radialGradient>
          <radialGradient id="aurora2" cx="15%" cy="-5%" r="45%">
            <stop offset="0%" stopColor="#9b72ff" stopOpacity="0.1" />
        ...
        {/* Clouds */}
        <Cloud x={110} y={42}  scale={1.1}  cls="cloud-1" />
        <Cloud x={480} y={76}  scale={0.85} cls="cloud-2" />
        <Cloud x={730} y={28}  scale={0.65} cls="cloud-3" />
        <Cloud x={300} y={-20} scale={0.5}  cls="cloud-2" />
        ...
        {ROCK_STROKES.map(([x1,y1,x2,y2], i) => (
          <line key={i} x1={x1} y1={y1} x2={x2} y2={y2}
            stroke="#1e2030" strokeWidth="2.5" opacity="0.28" strokeLinecap="round" />
        ))}

        {/* Mountain outline */}
        <path d={MAIN_MOUNTAIN} fill="none" stroke="#363a52" strokeWidth="1.5" opacity="0.6" />

        {/* Snow caps at each tier peak */}
        ...
        <polyline
          points={ascentPts}
          fill="none"
          stroke="rgba(244,214,80,0.65)"
          strokeWidth="2"
          strokeDasharray="8 7"
          className="trail-anim"
        />

        {/* Tier labels above each peak */}
      ...
      {/* Climber figures — positioned as HTML over the SVG */}
      {board.length === 0 && (
        <div className="empty" style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', pointerEvents:'none' }}>
      ...
      {laid.map(({ climber, pos }, idx) => {
        ...
        return (
          <div
            key={climber.name}
            className={`climber-tag${flipTip ? ' flip-tip' : ''}`}
          ...
          >
            ...
            <ClimberFigure
              teamColor={colorForTeam(climber.team)}
              name={climber.name}
              tierColor={tier.color}
              isLeader={idx === 0}
              size={28}
            />
            <div className="climber-label">
              {climber.points} pts{streak >= 2 ? ` · ${streak}wk` : ''}
            </div>
          </div>
        );
      })}
